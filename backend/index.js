...
app.post('/api/attendance/mark', authenticateToken, (req, res) => {
  ...
  verifyQRToken(qr_token, (err, decoded) => {
    if (err) return res.status(400).json({ error: 'Invalid or expired QR' });
    db.get('SELECT * FROM qr_tokens WHERE token = ? AND used = 0 AND expires_at > ?', [qr_token, new Date().toISOString()], (err, qrRow) => {
      if (err || !qrRow) return res.status(400).json({ error: 'QR already used or expired' });
      if (!checkGeofence(latitude, longitude)) return res.status(400).json({ error: 'Outside office radius' });
